...
return client.sendButton(msg.from,(await client.getBuffer(botinfo.linkApi.zacros+"/search/alphacoders?query="+query)).buffer,'imageMessage',[{id:`.pinterest ${query}`,text:'Get Again'}],{caption:functions.parseResult({title:'Alphacoders Search'}),footer:botinfo.footerText})
},{wait:true,query:`Masukkan Query Anda Untuk Info Ketik .alphacoders --info`,param:`${functions.needed('query')}`,info:'Mencarikan Foto Di Pinterest'})
