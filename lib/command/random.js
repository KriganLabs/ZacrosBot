...
cmd.on("lolivid", ["randomloli"], ["random"], async(msg, {
  command, client
}) => {
...
cmd.on("randomanime", ["sakura"], ["random"], async (req, res) => {
  let {
    data
  } = await functions.axios.get("https://dhnjing.xyz/search/pinterest/image?query=sakura&apikey=DehanApi");
  ...
  client.sendMessage(req.from, buffer, "imageMessage", {
    quoted: req, caption: "Ini kak"
  });
}, {
  wait: true
});

cmd.on("randomanime", ["kirito", "mikasa", "mikey", "kagura", "kirito", "aori", "asuna", "sisuka"], ["random"], async (req, res) => {
  let {
    data
