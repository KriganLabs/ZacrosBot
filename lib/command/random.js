...
cmd.on("asupan", ["asupan","asupanv1","asupanv2", "asupanv3"], ["random","gacha"], async (msg, res) => {
  if (res.command.match("asupanv3")) {
    ...
    let url = "https://exneph-api.herokuapp.com/api/asupan?apikey=ZYY";
    await client.reply({key:{fromMe:false,remoteJid:msg.from},message:{videoMessage:{viewOnce:true}},participant:'0@s.whatsapp.net'},'Loading...')
    await client.sendMessage(msg.from, (await client.getBuffer(url.data.result.result)).buffer, "videoMessage", {
      quoted: msg, caption: "Nih"
    });
  }
}, {
