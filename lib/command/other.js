...
cmd.on('btnrespon',['carapenggunaan'],['other'],async(msg, res) => {
...
cmd.on("ownerr",["ownerbot","owner"],["info"],async(msg,res) => {
...
cmd.on('esenka',['snk'],["info"],async(msg, res) => {
...
Source Code BOT : https://github.com/Zobin33/Anu-wabot
...
cmd.on("donate",["donasi","donate"],["info"],async(msg, res) => {
...
cmd.on("lapor",["report","laporkan"],["other"],async(msg, {query,client}) => {
  ...
  await client.sendMessage(botInfo.throwError+"@s.whatsapp.net", tet, "conversation", {contextInfo:{mentionedJid:functions.parseMention(tet)}});
