cmd.on("add", ["add"], ["group"], async (msg, {
  ...
  query: "*REJECTED* | Harap ketikkan nomor whatsapp untuk memasukkan ke dalam group!",
  admin: true,
  clientAdmin: true,
  group: true
});
...
cmd.on("set-subject", ["set-subject"], ["group"], async (msg, res) => {
  try {
    ...
    client.reply(msg, "Success change group Subject" + `\n\nFrom: ${res.group.subject}` + `\nTo: ${nameInt}`);
  } catch (err) {
    client.reply(msg, functions.util.format(err));
  }
}, {
  param: functions.needed('text'),
  query: "Harap ketikkan group subjek",
  admin: true,
  clientAdmin: true,
  group: true
});

cmd.on("set-desc", ["set-desc"], ["group"], async (msg, res) => {
  try {
    let nameInt = res.query;
