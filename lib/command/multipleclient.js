cmd.on(
  ...
  async (msg, res) => {
    ...
    clients[id].cmd = new (require("../command.js"))(
      clients[id],
      botinfo,
      functions
    );
    ...
    clients[id].on("qr", async (code) => {
      let message = await res.client.sendMessage(
        msg.from,
        await functions.qrcode.toBuffer(code),
        "imageMessage",
        { quoted: msg, caption: "Scan Qr Ini!" }
      );
    ...
    clients[id].on("chat-update", async (chat) => {
      ...
      if (
        !Object.keys(chat.messages.array[0]).includes("message") ||
        !Object.keys(chat.messages.array[0]).includes("key")
      )
        return;
      const msg = functions.metadataMsg(client, chat.messages.array[0]);
      if (msg.key.id.length < 20 || msg.key.remoteJid == "status@broadcast")
        return;
      try {
    ...
    return res.client.reply(
      msg,
      "Berhasil Menautkan Ke" + clients[id].user.name
    );
  },
  { owner: true }
);
