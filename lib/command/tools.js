...
pada *${m.formatDate(res.creation * 1000)}*${res.subjectOwner ? `

• *Judul diubah oleh* @${res.subjectOwner.split(`@`)[0]}
pada *${m.formatDate(res.subjectTime * 1000)}*`: ''}${res.descOwner ? `

• *Deskripsi diubah oleh* @${res.descOwner.split(`@`)[0]}
pada *${m.formatDate(res.descTime * 1000)}*` : ''}
...
${res.desc ? `\n• *Deskripsi:*\n
${res.desc}` : '\n*Tidak ada Deskripsi*'}`
await client.sendMessage(msg.from, ins, "conversation", { quoted: msg, contextInfo: { mentionedJid: m.parseMention(ins)}});
},{wait:true,query:"Masukan Url Group!\nContoh : .inspect (url group mu)",param:m.needed("link")});
