...
cmd.on('play',['play'],['downloader','search'],async(msg,{query,client,prefix}) => {
  ...
  let thumb = (await client.getBuffer(data.thumbnail)).buffer
  buttons = [{buttonId:`${prefix[0]}ttnowm ${data.nowatermark}`,buttonText:{displayText:'No Watermark'},type:1},{buttonId:`${prefix[0]}ttwm ${data.watermark}`,buttonText:{displayText:'Watermark'},type:1}]
    let locationMessage = {jpegThumbnail:logo.buffer}
    used_logo = (used_logo+1)%3
    let buttonsMessage = {
    contentText: `*Tiktok Downloader*\n\nNote :Jika Tidak Bisa Menekan Tombol, Ketik \n${prefix[0]}ttnowm ${data.nowatermark}`,
    footerText: '© Powered By Zacros Team',
    locationMessage,
    buttons,
    headerType: "LOCATION"
