...
cmd.on('play',['play'],['downloader','search'],async(msg,{query,client,prefix}) => {
  ...
  cmd.on('tiktok',['tiktok'],['downloader'],async(msg,{query,client,prefix}) => {
  
  let res = await functions.axios.get(`https://zacrosapi.herokuapp.com/downloader/tiktok?link=${query}`)
  let data = res.data.result
  let thumb = (await client.getBuffer(data.thumbnail)).buffer
  buttons = [{buttonId:`${prefix[0]}tiktoknowm ${data.nowatermark}`,buttonText:{displayText:'No Watermark'},type:1},{buttonId:`${prefix[0]}tiktokwm ${data.watermark}`,buttonText:{displayText:'Watermark'},type:1}]
    let locationMessage = {jpegThumbnail:logo.buffer}
    used_logo = (used_logo+1)%3
    let buttonsMessage = {
    contentText: `*Tiktok Downloader*\n\nNote :Jika Tidak Bisa Menekan Tombol, Ketik \n${prefix[0]}tiktoknowm ${data.nowatermark}`,
    footerText: '© Powered By Zacros Team',
    locationMessage,
    buttons,
    headerType: "LOCATION"
    ...
    return client.sendMessageFromContent(msg.from,{buttonsMessage})
    },{query:`Masukan link tiktok`,param:functions.needed('Link')})
