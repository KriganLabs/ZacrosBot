...
let anu = command.endsWith('doc') || command.endsWith('dokumen') || command.endsWith('doc') ? (await y2mateA(isUrl[0]))[0] : (await y2mateV(isUrl[0]))[0]
let res = command.endsWith('doc') || command.endsWith('dokumen') || command.endsWith('doc') ? {type:'documentMessage',mimetype:'audio/mpeg',filename:type.output,quoted:msg} : {type:'documentMessage',mimetype:'audio/mp3',filename:type.output,quoted:msg}
...
cmd.on('play',['play','playvid','playaud'],['downloader','search'],async(msg,{query,client,prefix}) => {
