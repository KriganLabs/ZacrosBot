...
const fs = require('fs')
const os = require('os'); 
//Modul nyata awokawokawokawok


const fetchJson = fetchJson = (url, options) => new Promise(async (resolve, reject) => {
    fetch(url, options)
...
num = fetchJson(`https://numlookupapi.com/api/validate/${senderNumber}`, {method: 'get'})
stst = client.getStatus(`${sender.split('@')[0]}@c.us`)
