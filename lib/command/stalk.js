...
cmd.on(
  ...
  async (msg, { query, client }) => {
    if (!query)
      return client.sendText(
        msg.from,
        `Isi Query!\nContoh : .igstalk creallizy.xd`,
        msg
      );
    let res = await functions.axios.get(
      "https://zenzapi.xyz/api/stalker/ig?username=" + query + "&apikey=Zacros"
    );
...
• Collections : ${data.collections}`;
    return client.sendMessage(
      msg.from,
      (await client.getBuffer(data.profilehd)).buffer,
      "imageMessage",
      { caption: isi, quoted: msg }
    );
  },
...
cmd.on(
  ...
  async (msg, { query, client }) => {
    ...
    let res = await functions.axios.get(
      "https://zenzapi.xyz/api/stalker/tiktok?username=" +
        query +
        "&apikey=Zacros"
    );
...
• Video : ${stats.videoCount}`;
    return client.sendMessage(
      msg.from,
      (await client.getBuffer(data.avatarLarger)).buffer,
      "imageMessage",
      { caption: isi, quoted: msg }
    );
  },
