...
cmd.on('bc',['broadcast','bc','broadcastgc','bcgc'],['owner'],(req,res) => {
if (!res.query) return client.sendText(req.from,"Masukkan query")

if (res.command.match["bc","broadcast"]) {
let anu = client.chats.all().filter(a => !a.read_only && a.message && !a.archive).map(a => a.jid)
for (let _ of anu) {
client.sendText(_.jid, `[ Bot Broadcast ]\n\n${res.query}`)}
}
if (res.command.match["bcgc","broadcastgc"]) {
let anu = client.chats.all().filter(v => v.jid.endsWith('g.us') && !v.read_only && v.message && !v.announce).map(v => v.jid)
for (let _ of anu) {
client.sendText(_, `[ All Group Broadcast ]\n\n${query}`)}
}
},{owner:'Only owner'});
