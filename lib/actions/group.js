client.on("group-participants-update", async (_events) => {
  ...
  let pp = await client.getProfilePicture(groupM.jid)
  switch (_events.action) {
    ...
    case "add": {
      for (let id of userId) {
        let txt = `Hai @${id.split("@")[0]} Selamat Datang\n\nTotal Member : ${groupM.participants.length}\nAdmin : ${groupM.groupAdmins.length}\n\nDesc Group :\n${groupM.desc}`;
        client.sendMessage(groupId, (await client.getBuffer(pp)).buffer,'imageMessage',{caption:txt, "conversation", { 
          contextInfo: {
            mentionedJid: userId
          }
        }});
