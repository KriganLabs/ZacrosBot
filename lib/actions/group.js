client.on("group-participants-update", async (_events) => {
  ...
  switch (_events.action) {
    ...
    case "add": {
      ...
      let anu = msg.groupData;
      client.sendMessage(groupId,`Hai @${id.split("@")[0]} Selamat Datang Ke ${anu.name}\n\nTotal Member : ${anu.participants.length}\nAdmin : ${anu.groupAdmins.length}\n\nDesc Group :\n${anu.desc}`,"conversation", { 
          contextInfo: {
            mentionedJid: userId
          }
        });
